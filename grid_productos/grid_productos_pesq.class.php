...
//-- ajax metodos ---
      ...
      if ($this->NM_ajax_opcao == 'autocomp_marca_producto')
      {
          $marca_producto = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_marca_producto($marca_producto);
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
      ...
      if ($this->NM_ajax_opcao == 'autocomp_nombre_producto')
      {
          $nombre_producto = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_nombre_producto($nombre_producto);
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
          foreach ($nmgp_def_dados as $Ind => $Lista)
      ...
      if ($this->NM_ajax_opcao == 'autocomp_referencia_producto')
      {
          $referencia_producto = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_referencia_producto($referencia_producto);
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
          foreach ($nmgp_def_dados as $Ind => $Lista)
   ...
   function lookup_ajax_marca_producto($marca_producto)
   {
      $marca_producto = substr($this->Db->qstr($marca_producto), 1, -1);
            $marca_producto_look = substr($this->Db->qstr($marca_producto), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct Marca_Producto from " . $this->Ini->nm_tabela . " where  Marca_Producto like '%" . $marca_producto . "%'"; 
   ...
   function lookup_ajax_nombre_producto($nombre_producto)
   {
      $nombre_producto = substr($this->Db->qstr($nombre_producto), 1, -1);
            $nombre_producto_look = substr($this->Db->qstr($nombre_producto), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct Nombre_Producto from " . $this->Ini->nm_tabela . " where  Nombre_Producto like '%" . $nombre_producto . "%'"; 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      { 
   ...
   function lookup_ajax_referencia_producto($referencia_producto)
   {
      $referencia_producto = substr($this->Db->qstr($referencia_producto), 1, -1);
            $referencia_producto_look = substr($this->Db->qstr($referencia_producto), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct Referencia_Producto from " . $this->Ini->nm_tabela . " where  Referencia_Producto like '%" . $referencia_producto . "%'"; 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      { 
...
<HTML>
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_srch_titl'] ?> - productos</TITLE>
...
function NM_hide_menu()
{
 ...
 function nm_save_form(pos)
 {
  ...
  var str_out = "";
  str_out += 'SC_marca_producto_cond#NMF#' + search_get_sel_txt('SC_marca_producto_cond') + '@NMF@';
  str_out += 'SC_marca_producto#NMF#' + search_get_text('SC_marca_producto') + '@NMF@';
  str_out += 'id_ac_marca_producto#NMF#' + search_get_text('id_ac_marca_producto') + '@NMF@';
  ...
  str_out += 'SC_nombre_producto_cond#NMF#' + search_get_sel_txt('SC_nombre_producto_cond') + '@NMF@';
  str_out += 'SC_nombre_producto#NMF#' + search_get_text('SC_nombre_producto') + '@NMF@';
  str_out += 'id_ac_nombre_producto#NMF#' + search_get_text('id_ac_nombre_producto') + '@NMF@';
  str_out += 'SC_referencia_producto_cond#NMF#' + search_get_sel_txt('SC_referencia_producto_cond') + '@NMF@';
  str_out += 'SC_referencia_producto#NMF#' + search_get_text('SC_referencia_producto') + '@NMF@';
  str_out += 'id_ac_referencia_producto#NMF#' + search_get_text('id_ac_referencia_producto') + '@NMF@';
...
<script type="text/javascript">
 $(function() {
   ...
   $("#id_ac_nombre_producto").autocomplete({
     source: function (request, response) {
     $.ajax({
       url: "index.php",
       dataType: "json",
       data: {
          q: request.term,
          nmgp_opcao: "ajax_autocomp",
          nmgp_parms: "NM_ajax_opcao?#?autocomp_nombre_producto",
          max_itens: "10",
          cod_desc: "N",
          script_case_init: <?php echo $this->Ini->sc_page ?>
        },
     ...
     select: function (event, ui) {
       $("#SC_nombre_producto").val(ui.item.value);
       $(this).val(ui.item.label);
       event.preventDefault();
     },
     change: function (event, ui) {
       if (null == ui.item) {
          $("#SC_nombre_producto").val( $(this).val() );
       }
     }
   });
   $("#id_ac_referencia_producto").autocomplete({
     source: function (request, response) {
     $.ajax({
       url: "index.php",
       dataType: "json",
       data: {
          q: request.term,
          nmgp_opcao: "ajax_autocomp",
          nmgp_parms: "NM_ajax_opcao?#?autocomp_referencia_producto",
          max_itens: "10",
          cod_desc: "N",
          script_case_init: <?php echo $this->Ini->sc_page ?>
        },
     ...
     select: function (event, ui) {
       $("#SC_referencia_producto").val(ui.item.value);
       $(this).val(ui.item.label);
       event.preventDefault();
     },
     change: function (event, ui) {
       if (null == ui.item) {
          $("#SC_referencia_producto").val( $(this).val() );
       }
     }
   });
   $("#id_ac_marca_producto").autocomplete({
     source: function (request, response) {
     $.ajax({
       url: "index.php",
       dataType: "json",
       data: {
          q: request.term,
          nmgp_opcao: "ajax_autocomp",
          nmgp_parms: "NM_ajax_opcao?#?autocomp_marca_producto",
          max_itens: "10",
          cod_desc: "N",
          script_case_init: <?php echo $this->Ini->sc_page ?>
        },
     ...
     select: function (event, ui) {
       $("#SC_marca_producto").val(ui.item.value);
       $(this).val(ui.item.label);
       event.preventDefault();
     },
     change: function (event, ui) {
       if (null == ui.item) {
          $("#SC_marca_producto").val( $(this).val() );
       }
     }
   });
 });
...
<div style="width: 100%">
 ...
 <div style="height:37px; border-width:0px 0px 1px 0px;  border-style: dashed; border-color:#ddd; display: block">
 	<table style="width:100%; border-collapse:collapse; padding:0;">
    	<tr>
        	<td id="lin1_col1" class="scFilterHeaderFont"><span><?php echo $this->Ini->Nm_lang['lang_othr_srch_titl'] ?> - productos</span></td>
...
<?php
   ...
   {
      global 
             $marca_producto_cond, $marca_producto, $marca_producto_autocomp,
             $id_producto_cond, $id_producto, $id_producto_input_2, $id_producto_autocomp,
             $nombre_producto_cond, $nombre_producto, $nombre_producto_autocomp,
             $referencia_producto_cond, $referencia_producto, $referencia_producto_autocomp,
      ...
      { 
          ...
          $marca_producto = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['campos_busca']['marca_producto']; 
          $marca_producto_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['campos_busca']['marca_producto_cond']; 
          $id_producto = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['campos_busca']['id_producto']; 
          $id_producto_input_2 = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['campos_busca']['id_producto_input_2']; 
          $id_producto_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['campos_busca']['id_producto_cond']; 
          $nombre_producto = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['campos_busca']['nombre_producto']; 
          $nombre_producto_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['campos_busca']['nombre_producto_cond']; 
          $referencia_producto = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['campos_busca']['referencia_producto']; 
          $referencia_producto_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['campos_busca']['referencia_producto_cond']; 
          $this->NM_operador = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['campos_busca']['NM_operador']; 
      } 
      if (!isset($marca_producto_cond) || empty($marca_producto_cond))
      {
         $marca_producto_cond = "qp";
      }
      if (!isset($id_producto_cond) || empty($id_producto_cond))
      {
         $id_producto_cond = "gt";
      }
      if (!isset($nombre_producto_cond) || empty($nombre_producto_cond))
      {
         $nombre_producto_cond = "qp";
      }
      if (!isset($referencia_producto_cond) || empty($referencia_producto_cond))
      {
         $referencia_producto_cond = "qp";
      }
      $display_aberto  = "style=display:";
      $display_fechado = "style=display:none";
      $opc_hide_input = array("nu","nn","ep","ne");
      $str_hide_marca_producto = (in_array($marca_producto_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_id_producto = (in_array($id_producto_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_nombre_producto = (in_array($nombre_producto_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_referencia_producto = (in_array($referencia_producto_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;

      $str_display_id_producto = ('bw' == $id_producto_cond) ? $display_aberto : $display_fechado;

      if (!isset($marca_producto) || $marca_producto == "")
      {
          $marca_producto = "";
      }
      if (isset($marca_producto) && !empty($marca_producto))
      {
         $tmp_pos = strpos($marca_producto, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $marca_producto = substr($marca_producto, 0, $tmp_pos);
         }
      }
      ...
      if (isset($nombre_producto) && !empty($nombre_producto))
      {
         $tmp_pos = strpos($nombre_producto, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $nombre_producto = substr($nombre_producto, 0, $tmp_pos);
         }
      }
      if (!isset($referencia_producto) || $referencia_producto == "")
      {
          $referencia_producto = "";
      }
      if (isset($referencia_producto) && !empty($referencia_producto))
      {
         $tmp_pos = strpos($referencia_producto, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $referencia_producto = substr($referencia_producto, 0, $tmp_pos);
         }
      }
?>
 <?php
 ...
 <TR align="center">
  ...
  <TD width="100%" height="">
   ...
   <tr>
      ...
      <TD class="scFilterLabelOdd"><?php echo (isset($this->New_label['marca_producto'])) ? $this->New_label['marca_producto'] : "Marca Producto"; ?></TD>
     <TD class="scFilterFieldOdd"> 
      <SELECT class="scFilterObjectOdd" id="SC_marca_producto_cond" name="marca_producto_cond" onChange="nm_campos_between(document.getElementById('id_vis_marca_producto'), this, 'marca_producto')">
       <OPTION value="qp" <?php if ("qp" == $marca_producto_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_like'] ?></OPTION>
       <OPTION value="np" <?php if ("np" == $marca_producto_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_not_like'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $marca_producto_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="ep" <?php if ("ep" == $marca_producto_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_empty'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_marca_producto" <?php echo $str_hide_marca_producto?> valign="top">
        <TD class="scFilterFieldFontOdd">
           <?php
 $SC_Label = (isset($this->New_label['marca_producto'])) ? $this->New_label['marca_producto'] : "Marca Producto";
 $nmgp_tab_label .= "marca_producto?#?" . $SC_Label . "?@?";
 $date_sep_bw = "";
?>
<?php
      $marca_producto_look = substr($this->Db->qstr($marca_producto), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct Marca_Producto from " . $this->Ini->nm_tabela . " where Marca_Producto = '$marca_producto_look'"; 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      { 
      ...
      if (isset($nmgp_def_dados[0][$marca_producto]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$marca_producto];
      }
      else
      {
          $sAutocompValue = $marca_producto;
      }
?>
<INPUT  type="text" id="SC_marca_producto" name="marca_producto" value="<?php echo NM_encode_input($marca_producto) ?>" size=40 alt="{datatype: 'text', maxLength: 40, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectOdd" type="text" id="id_ac_marca_producto" name="marca_producto_autocomp" size="40" value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 40, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
...
<input class="sc-js-input scFilterObjectEven" type="text" id="id_ac_id_producto" name="id_producto_autocomp" size="11" value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'integer', maxLength: 11, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}">
      ...
      <TD class="scFilterLabelOdd"><?php echo (isset($this->New_label['nombre_producto'])) ? $this->New_label['nombre_producto'] : "Nombre Producto"; ?></TD>
     <TD class="scFilterFieldOdd"> 
      <SELECT class="scFilterObjectOdd" id="SC_nombre_producto_cond" name="nombre_producto_cond" onChange="nm_campos_between(document.getElementById('id_vis_nombre_producto'), this, 'nombre_producto')">
       <OPTION value="qp" <?php if ("qp" == $nombre_producto_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_like'] ?></OPTION>
       <OPTION value="np" <?php if ("np" == $nombre_producto_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_not_like'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $nombre_producto_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="ep" <?php if ("ep" == $nombre_producto_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_empty'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_nombre_producto" <?php echo $str_hide_nombre_producto?> valign="top">
        <TD class="scFilterFieldFontOdd">
           <?php
 $SC_Label = (isset($this->New_label['nombre_producto'])) ? $this->New_label['nombre_producto'] : "Nombre Producto";
 $nmgp_tab_label .= "nombre_producto?#?" . $SC_Label . "?@?";
 $date_sep_bw = "";
?>
<?php
      $nombre_producto_look = substr($this->Db->qstr($nombre_producto), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct Nombre_Producto from " . $this->Ini->nm_tabela . " where Nombre_Producto = '$nombre_producto_look'"; 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      { 
      ...
      if (isset($nmgp_def_dados[0][$nombre_producto]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$nombre_producto];
      }
      else
      {
          $sAutocompValue = $nombre_producto;
      }
?>
<INPUT  type="text" id="SC_nombre_producto" name="nombre_producto" value="<?php echo NM_encode_input($nombre_producto) ?>" size=20 alt="{datatype: 'text', maxLength: 20, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectOdd" type="text" id="id_ac_nombre_producto" name="nombre_producto_autocomp" size="20" value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 20, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
      ...
      <TD class="scFilterLabelEven"><?php echo (isset($this->New_label['referencia_producto'])) ? $this->New_label['referencia_producto'] : "Referencia Producto"; ?></TD>
     <TD class="scFilterFieldEven"> 
      <SELECT class="scFilterObjectEven" id="SC_referencia_producto_cond" name="referencia_producto_cond" onChange="nm_campos_between(document.getElementById('id_vis_referencia_producto'), this, 'referencia_producto')">
       <OPTION value="qp" <?php if ("qp" == $referencia_producto_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_like'] ?></OPTION>
       <OPTION value="np" <?php if ("np" == $referencia_producto_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_not_like'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $referencia_producto_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="ep" <?php if ("ep" == $referencia_producto_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_empty'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldEven">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_referencia_producto" <?php echo $str_hide_referencia_producto?> valign="top">
        <TD class="scFilterFieldFontEven">
           <?php
 $SC_Label = (isset($this->New_label['referencia_producto'])) ? $this->New_label['referencia_producto'] : "Referencia Producto";
 $nmgp_tab_label .= "referencia_producto?#?" . $SC_Label . "?@?";
 $date_sep_bw = "";
?>
<?php
      $referencia_producto_look = substr($this->Db->qstr($referencia_producto), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct Referencia_Producto from " . $this->Ini->nm_tabela . " where Referencia_Producto = '$referencia_producto_look'"; 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      { 
      ...
      if (isset($nmgp_def_dados[0][$referencia_producto]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$referencia_producto];
      }
      else
      {
          $sAutocompValue = $referencia_producto;
      }
?>
<INPUT  type="text" id="SC_referencia_producto" name="referencia_producto" value="<?php echo NM_encode_input($referencia_producto) ?>" size=20 alt="{datatype: 'text', maxLength: 20, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectEven" type="text" id="id_ac_referencia_producto" name="referencia_producto_autocomp" size="20" value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 20, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
...
?>
 ...
 function limpa_form()
 {
   ...
   nm_campos_between(document.getElementById('id_vis_marca_producto'), document.F1.marca_producto_cond, 'marca_producto');
   document.F1.marca_producto.value = "";
   document.F1.marca_producto_autocomp.value = "";
   ...
   nm_campos_between(document.getElementById('id_vis_nombre_producto'), document.F1.nombre_producto_cond, 'nombre_producto');
   document.F1.nombre_producto.value = "";
   document.F1.nombre_producto_autocomp.value = "";
   nm_campos_between(document.getElementById('id_vis_referencia_producto'), document.F1.referencia_producto_cond, 'referencia_producto');
   document.F1.referencia_producto.value = "";
   document.F1.referencia_producto_autocomp.value = "";
 }
...
<?php
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "inversoft/grid_productos";
   ...
   function recupera_filtro()
   {
      ...
      $tp_fields     = array();
      $tb_fields_esp = array();
      $tp_fields['SC_marca_producto_cond'] = 'cond';
      $tp_fields['SC_marca_producto'] = 'text_aut';
      $tp_fields['id_ac_marca_producto'] = 'text_aut';
      ...
      $tp_fields['SC_nombre_producto_cond'] = 'cond';
      $tp_fields['SC_nombre_producto'] = 'text_aut';
      $tp_fields['id_ac_nombre_producto'] = 'text_aut';
      $tp_fields['SC_referencia_producto_cond'] = 'cond';
      $tp_fields['SC_referencia_producto'] = 'text_aut';
      $tp_fields['id_ac_referencia_producto'] = 'text_aut';
      if (is_file($NM_patch))
      {
          $SC_V8    = false;
   ...
   {
      global $marca_producto_cond, $marca_producto, $marca_producto_autocomp,
             $id_producto_cond, $id_producto, $id_producto_input_2, $id_producto_autocomp,
             $nombre_producto_cond, $nombre_producto, $nombre_producto_autocomp,
             $referencia_producto_cond, $referencia_producto, $referencia_producto_autocomp, $nmgp_tab_label;

      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_gp_limpa.php", "F", "nm_limpa_valor") ; 
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_conv_dados.php", "F", "nm_conv_limpa_dado") ; 
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_edit.php", "F", "nmgp_Form_Num_Val") ; 
      if (!empty($marca_producto_autocomp) && empty($marca_producto))
      {
          $marca_producto = $marca_producto_autocomp;
      }
      if (!empty($id_producto_autocomp) && empty($id_producto))
      {
          $id_producto = $id_producto_autocomp;
      }
      if (!empty($nombre_producto_autocomp) && empty($nombre_producto))
      {
          $nombre_producto = $nombre_producto_autocomp;
      }
      if (!empty($referencia_producto_autocomp) && empty($referencia_producto))
      {
          $referencia_producto = $referencia_producto_autocomp;
      }
      $marca_producto_cond_salva = $marca_producto_cond; 
      if (!isset($marca_producto_input_2) || $marca_producto_input_2 == "")
      {
          $marca_producto_input_2 = $marca_producto;
      }
      ...
      $nombre_producto_cond_salva = $nombre_producto_cond; 
      if (!isset($nombre_producto_input_2) || $nombre_producto_input_2 == "")
      {
          $nombre_producto_input_2 = $nombre_producto;
      }
      $referencia_producto_cond_salva = $referencia_producto_cond; 
      if (!isset($referencia_producto_input_2) || $referencia_producto_input_2 == "")
      {
          $referencia_producto_input_2 = $referencia_producto;
      }
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['campos_busca']['marca_producto'] = $marca_producto; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['campos_busca']['marca_producto_cond'] = $marca_producto_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['campos_busca']['id_producto'] = $id_producto; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['campos_busca']['id_producto_input_2'] = $id_producto_input_2; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['campos_busca']['id_producto_cond'] = $id_producto_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['campos_busca']['nombre_producto'] = $nombre_producto; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['campos_busca']['nombre_producto_cond'] = $nombre_producto_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['campos_busca']['referencia_producto'] = $referencia_producto; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['campos_busca']['referencia_producto_cond'] = $referencia_producto_cond_salva; 
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['marca_producto'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          ...
          $this->cmp_formatado['marca_producto'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['marca_producto'] = $marca_producto;
      }
      $id_producto_look = substr($this->Db->qstr($id_producto), 1, -1); 
      $nmgp_def_dados = array(); 
   if (is_numeric($id_producto))
   { 
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['nombre_producto'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          ...
          $this->cmp_formatado['nombre_producto'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['nombre_producto'] = $nombre_producto;
      }
      $referencia_producto_look = substr($this->Db->qstr($referencia_producto), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct Referencia_Producto from " . $this->Ini->nm_tabela . " where Referencia_Producto = '$referencia_producto_look'"; 
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['referencia_producto'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          ...
          $this->cmp_formatado['referencia_producto'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['referencia_producto'] = $referencia_producto;
      }

      //----- $marca_producto
      $this->Date_part = false;
      if (isset($marca_producto) || $marca_producto_cond == "nu" || $marca_producto_cond == "nn" || $marca_producto_cond == "ep" || $marca_producto_cond == "ne")
      {
         $this->monta_condicao("Marca_Producto", $marca_producto_cond, $marca_producto, "", "marca_producto");
      }
      ...
      //----- $nombre_producto
      $this->Date_part = false;
      if (isset($nombre_producto) || $nombre_producto_cond == "nu" || $nombre_producto_cond == "nn" || $nombre_producto_cond == "ep" || $nombre_producto_cond == "ne")
      {
         $this->monta_condicao("Nombre_Producto", $nombre_producto_cond, $nombre_producto, "", "nombre_producto");
      }

      //----- $referencia_producto
      $this->Date_part = false;
      if (isset($referencia_producto) || $referencia_producto_cond == "nu" || $referencia_producto_cond == "nn" || $referencia_producto_cond == "ep" || $referencia_producto_cond == "ne")
      {
         $this->monta_condicao("Referencia_Producto", $referencia_producto_cond, $referencia_producto, "", "referencia_producto");
      }
   }
   ...
   function finaliza_resultado()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_productos']['where_pesq_ant'] = $this->comando . $this->comando_fim;
